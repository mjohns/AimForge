...
SDL_Surface *IMG_LoadSTB_IO(SDL_IOStream *src) {
  ...
  if (SDL_ReadIO(src, magic, sizeof(magic)) == sizeof(magic)) {
    ...
    if (magic[0] == 0x89 && magic[1] == 'P' && magic[2] == 'N' && magic[3] == 'G' &&
        magic[12] == 'I' && magic[13] == 'H' && magic[14] == 'D' && magic[15] == 'R' &&
  ...
  if (use_palette) {
    ...
    pixels = stbi_load_from_callbacks_with_palette(
        &rw_callbacks, src, &w, &h, palette_colors, SDL_arraysize(palette_colors));
  } else {
    pixels = stbi_load_from_callbacks(&rw_callbacks, src, &w, &h, &format, STBI_default);
  }
  if (!pixels) {
    SDL_SeekIO(src, start, SDL_IO_SEEK_SET);
    return NULL;
  ...
  if (use_palette) {
    surface = SDL_CreateSurfaceFrom(w, h, SDL_PIXELFORMAT_INDEX8, pixels, w);
    if (surface) {
    ...
    surface = SDL_CreateSurfaceFrom(w,
                                    h,
